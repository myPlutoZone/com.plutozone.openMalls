...
<%@ page info="/WEB-INF/view/seller/product/modifyForm.jsp" %>
	...
	<script>
		...
		*/
		function modifyProc() {
			...
			frmMain.action="/seller/product/modifyProc.web";
			frmMain.submit();
		}
	</script>
</head>
				...
				<tr>
					...
					<td>
						<select name="cd_state_prd" id="cd_state_prd">
							<option value="1"<c:if test="${list.cd_state_prd == '1'}"> checked</c:if>>판매중</option>
							<option value="2"<c:if test="${list.cd_state_prd == '2'}"> checked</c:if>>판매중지</option>
							<option value="3"<c:if test="${list.cd_state_prd == '3'}"> checked</c:if>>반려</option>
							<option value="9"<c:if test="${list.cd_state_prd == '9'}"> checked</c:if>>재고소진</option>
						</select>
					</td>
				</tr>
				<tr>
					<th>카테고리(*)</th>
					<td>
						<select id="cd_ctg" name="cd_ctg">
							<option value="01"<c:if test="${productDto.cd_ctg == '01'}"> selected</c:if>>총류</option>
							<option value="02"<c:if test="${productDto.cd_ctg == '02'}"> selected</c:if>>철학</option>
							<option value="03"<c:if test="${productDto.cd_ctg == '03'}"> selected</c:if>>종교</option>
							<option value="04"<c:if test="${productDto.cd_ctg == '04'}"> selected</c:if>>사회</option>
							<option value="05"<c:if test="${productDto.cd_ctg == '05'}"> selected</c:if>>자연</option>
							<option value="06"<c:if test="${productDto.cd_ctg == '06'}"> selected</c:if>>기술</option>
							<option value="07"<c:if test="${productDto.cd_ctg == '07'}"> selected</c:if>>예술</option>
							<option value="08"<c:if test="${productDto.cd_ctg == '08'}"> selected</c:if>>언어</option>
							<option value="09"<c:if test="${productDto.cd_ctg == '09'}"> selected</c:if>>문학</option>
						</select>
					</td>
				</tr>
				<tr>
